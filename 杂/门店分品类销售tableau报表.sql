...
employee_code
,time
,substring(time,1,10) as record_date
,substring(time,12,8) as record_time
,case when substring(time,12,8) between '00:00:00' and '03:59:59' then date_sub(substring(time,1,10),1) else substring(time,1,10) end as real_work_day
,row_number()over(partition by case when substring(time,12,8) between '00:00:00' and '03:59:59' then date_sub(substring(time,1,10),1) else substring(time,1,10) end order by time desc) as rn
,row_number()over(partition by case when substring(time,12,8) between '00:00:00' and '03:59:59' then date_sub(substring(time,1,10),1) else substring(time,1,10) end order by time ) as rn_1
from default.pdw_git_ehr_attendance_attendance_clock_record
where dt = 20231006
and employee_code in ('11299948')
),
...
a.employee_code
,a.real_work_day
,a.time as go_work_time
,b.time as out_work_time
from go_work_list a left join out_work_list b on a.employee_code = b.employee_code and a.real_work_day = b.real_work_day
--and a.real_work_day between '2023-09-01' and '2023-09-30'
...
left join score_list b on a.record_week = b.record_week and a.store_code = b.store_code and a.bool = b.bool and b.dt = date_format(date_sub(current_date(),1),'yyyyMMdd')
