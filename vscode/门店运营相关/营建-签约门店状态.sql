--营建-签约门店状态
--data_build.dwd_store_construction_project_status_v2_di
...
(
	...
	left join data_build.dim_store_construction_project_info t2 on t1.flag_code = t2.flag_code and t2.dt = '${DATE}'
...
(
	...
	(
		...
		*
		...
		from data_build.dwa_store_construction_project_signed_opening_schedure --CY15-2733对应2个store_id =('887','154')（887工程记录是全的，似乎因为续约）
		where dt = '${DATE}'
	) t1
	left join
	(
...
/*	select
	...
	,version
	,'1' is_delete
	from data_build.ods_uploads_internal_closure_pipeline_v2
...
(
	...
	(
		...
		from data_build.pdw_opc_flag_seal_apply_info t2
		where dt='${DATE}'
