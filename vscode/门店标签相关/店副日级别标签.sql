--店长标签日级别表 --0304更新 --data_build.dwd_manager_tag_v1_di
with 
...
,b.change_date0
,t.alarm_start_date
,cast(substr(t.final_level_modify,2,1) as double) as t_level
from base_manager_info b
join data_shop.dwd_ic_new_import_store_level_da_view t
  on b.store_code = t.shop_id
  and t.dt = '$(date -d "-1 day $context_date" "+%Y%m%d")'
where datediff(t.alarm_start_date, b.change_date0) >= 8
  and datediff(t.alarm_start_date, b.change_date0) <= 14
),
baseline_t as (
select 
employee_id
,store_code
,avg(t_level) as baseline_t
from baseline_t_raw
group by employee_id, store_code
),
...
t1.store_code
,t1.c_date
from opening_days_base t1
where t1.is_start_7 = 1
and t1.c_date >= '$(date -d "-7 day $context_date" "+%Y-%m-%d")'
and t1.c_date <= '$(date -d "-1 day $context_date" "+%Y-%m-%d")'
),
t7_final as (
select 
t1.store_code
,avg(t2.final_t_level) as t7_avg
from opening_days_7 t1
left join t_byday t2 on t1.store_code = t2.shop_id and t1.c_date = t2.alarm_start_date
where t2.is_start_7 = 1
group by t1.store_code
),
...
,bm.store_code
,bm.change_date0
,datediff(from_unixtime(unix_timestamp('$(date -d "-1 day $context_date" "+%Y%m%d")','yyyyMMdd')), bm.change_date0) as days_since_takeover
,tf.final_t_level as t30
,t7.t7_avg
,bl.baseline_t
-- f_score
,case when tf.final_t_level <= 1.6 then 5.0 when tf.final_t_level <= 2.0 then 4.0 when tf.final_t_level <= 2.5 then 3.0 when tf.final_t_level <= 4.0 then 2.0 else 1.0 end as f_score
-- delta
,coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level) as delta_t
-- g_score_raw
,case when (coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) >= 2.0 then 5.0
  when (coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) >= 1.0 then 4.0
  when (coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) >= 0.3 then 3.5
  when abs(coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) < 0.3 then 3.0
  when (coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) <= -2.0 then 1.0
  when (coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) <= -1.0 then 2.0
  when (coalesce(bl.baseline_t, tf.final_t_level) - coalesce(t7.t7_avg, tf.final_t_level)) <= -0.3 then 2.5
  else 3.0 end as g_score_raw
-- is_good_store
,case when tf.final_t_level <= 2.0 or t7.t7_avg <= 2.0 then 1 else 0 end as is_good_store
-- decay
,case when datediff(from_unixtime(unix_timestamp('$(date -d "-1 day $context_date" "+%Y%m%d")','yyyyMMdd')), bm.change_date0) > 180
  and not (tf.final_t_level <= 2.0 or coalesce(t7.t7_avg, tf.final_t_level) <= 2.0)
  then greatest(0.3, least(1.0, 1.0 - (datediff(from_unixtime(unix_timestamp('$(date -d "-1 day $context_date" "+%Y%m%d")','yyyyMMdd')), bm.change_date0) - 180) / 360.0))
  else 1.0 end as decay
from base_manager_info bm
left join t_final tf on bm.store_code = tf.store_code
left join t7_final t7 on bm.store_code = t7.store_code
left join baseline_t bl on bm.employee_id = bl.employee_id
),

-- fair_t 最终值 (含好门店保护+安全阀)
fair_t_final as (
select
fc.employee_id
,fc.store_code
...
,fc.is_good_store
,fc.decay
,fc.days_since_takeover
-- g_adj
,case when fc.is_good_store = 1 then greatest(fc.g_score_raw, 5.0)
  else fc.g_score_raw * fc.decay + 3.0 * (1.0 - fc.decay) end as g_adj
-- fair_t_raw
,0.7 * fc.f_score + 0.3 * (case when fc.is_good_store = 1 then greatest(fc.g_score_raw, 5.0) else fc.g_score_raw * fc.decay + 3.0 * (1.0 - fc.decay) end) as fair_t_raw
-- fair_t: 好门店保护(fair_t不低于原始t_score) + 安全阀待在final_list2中应用(需要will/perf)
,case when fc.is_good_store = 1
  and 0.7 * fc.f_score + 0.3 * greatest(fc.g_score_raw, 5.0) < (case when fc.t30 <= 1.6 then 5.0 when fc.t30 <= 2.0 then 4.0 when fc.t30 <= 2.5 then 3.0 when fc.t30 <= 4.0 then 2.0 else 1.0 end)
  then (case when fc.t30 <= 1.6 then 5.0 when fc.t30 <= 2.0 then 4.0 when fc.t30 <= 2.5 then 3.0 when fc.t30 <= 4.0 then 2.0 else 1.0 end)
  else 0.7 * fc.f_score + 0.3 * (case when fc.is_good_store = 1 then greatest(fc.g_score_raw, 5.0) else fc.g_score_raw * fc.decay + 3.0 * (1.0 - fc.decay) end)
end as final_t_level --即fair_t
from fair_t_calc fc
),

-- ========== Fair T-Value v3.0 新增结束 ==========
-- 品控
audit_base as (
select 
shop_code
...
left join data_build.dim_user_hr t1 on t0.employee_uid = t1.user_name and t1.dt = '${today-1}'
where t0.dt = '${today-1}'
and user_job = '门店稽核') a
where rn = 1
group by
...
left join fair_t_final t13 on t0.store_code = t13.store_code
...
-- 门店质量
,least(5.0, greatest(1.0, final_t_level)) as t_score -- t档分数 基础分
-- 客诉分数 扣分
