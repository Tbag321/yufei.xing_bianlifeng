...
high_freq_user as(
select a.user_id
,a.order_cnt as order_cnt_m5--5月单量
,sum(coalesce(b.order_cnt,0)) as order_cnt--30天内单量
from
(select user_id
...
from data_md.dm_order_summary_soberhi_order_detail_user_store_sku_day_di_v1 a
    ...
    join high_freq_user d on a.user_id=d.user_id and d.order_cnt=0
  where a.dt >= '20220501'
    and a.dt<='20220531'
    and a.sku_quantity > 0
    and coalesce(a.pay_id,'') not in('','30112507801894')
...
select a.order_cnt_m5,
b.order_no_num,
c.order_no_num,
d.order_no_num,
,count(distinct a.user_id) as total_user_cnt
,count(distinct case when a.order_cnt>0 then a.user_id end) as rebuy_user_cnt
from high_freq_user a
left join m5_list b on a.user_id=b.user_id and b.sku_division_name='咖啡'
left join m5_list c on a.user_id=c.user_id and c.sku_division_name='茶饮'
left join m5_list d on a.user_id=d.user_id and d.sku_division_name='其它'
group by 1,2,3
