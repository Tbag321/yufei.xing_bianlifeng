...
SELECT order_no as `订单号`,order_date as `订单日期`,user_id as `user_id`,b.store_cvs_code as `点位code`,b.display_name as `点位名称`,sku_code as `商品code`,sku_name as `商品名称`,sku_quantity as `商品数量`,sell_price as `零售价`,payable_price as `支付金额`,discount_price as `优惠金额`,refund_price as `退款金额` 
...
and b.store_cvs_code in ('123029744')
and order_date BETWEEN '2023-10-01' and '2023-11-23'
...
WHERE dt = '20231108' 
and shop_code in ('123028799','123027700') 
and record_date between '2022-05-01' and '2023-12-10' 
...
WHERE dt = '20220730' 
and shop_code in ('4f490a6dba1e972070c49bcd0a173101') 
and record_date between '2021-08-01' and '2022-06-30' 
...
and b.store_cvs_code in ('100121776')
and a.pay_status = 'PAY_SUCCESS'
and a.order_date between '2023-01-01' and '2024-02-05'
...
SELECT count(DISTINCT(order_no)) as `订单数`,sum(payable_price) as `销售总额`,sum(sku_quantity) as `商品数量`,b.store_cvs_code as `点位code`,b.display_name as `点位名称`
from default.dw_order_sku_v1_view_us_ab a
left join desensitization b on a.store_code = b.store_code 
WHERE dt = '20221107' 
and order_business_type = 'SELFTAKE' 
and pay_status = 'PAY_SUCCESS' 
and b.store_cvs_code in ('100151704','100150733','100150734') 
and order_date BETWEEN '2022-08-01' and '2022-10-31'
GROUP BY b.store_cvs_code,b.display_name
