...
user_type as(
 ...
 (select case when date(a.order_date)>=date_parse('20220303','%%Y%%m%%d') and date(a.order_date)<date_add('day',28,date_parse('20220303','%%Y%%m%%d')) then '0303-0330'
    ...
    ,b.sku_division_name,c.exp_group,a.pay_id as user_id from data_md.dm_order_summary_soberhi_order_detail_user_store_sku_day_di_v1 a
...
user_uad as (
    ...
    ,case when count(distinct case when c.is_new=1 then b.user_id end)=0 then 0 else count(distinct case when c.is_new=0 then concat(b.user_id,cast(b.order_date as varchar)) end)*1.0/count(distinct case when c.is_new=0 then b.user_id end) end as uad_28_old
    ...
    left join user_type c on a.date_week=c.date_week and b.exp_group=c.exp_group and b.sku_division_name=c.sku_division_name and b.user_id=c.user_id
    group by 1,2,3,4
),
